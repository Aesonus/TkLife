...
class TestCommandHistory:
    ...
    def test_add_history_adds_command_and_calls_command_execute(self, mock_command: Mock):
        command_history = CommandHistory()
    ...
    def test_undo_calls_command_reverse(self, cursor, command_history: CommandHistory, mock_command_list: list[Mock]):
        # Setup
    ...
    def test_undo_all_calls_command_reverse_on_history_until(self, until, command_history: CommandHistory, mock_command_list: list[Mock]):
    ...
    def test_redo_calls_command_execute(self, cursor, command_history: CommandHistory, mock_command_list: list[Mock]):
        command_history.cursor = cursor
        command_history.redo()
        mock_command_list[cursor +
                          1 if cursor is not None else 0].execute.assert_called_once_with()

    def test_redo_calls_no_command_if_cursor_maxed(self, command_history, mock_command_list):
    ...
    @pytest.mark.parametrize("range_size, cursor, expected", [
        (0, None, 0),
        (0, 0, 0),
        (1, 0, 1),
        (1, 0, 1),
    ...
    def test_len_returns_history_length_that_can_be_undone(self, range_size, cursor, expected, mocker: MockerFixture):
        command_history = CommandHistory()
        command_history.history = [mocker.Mock(
            Command) for __ in range(range_size)]
        command_history.cursor = cursor

        assert len(command_history) == expected

    def test_add_command_clears_redo_commands(self, command_history: CommandHistory, mock_command_list: list[Mock], mocker):
